...
class GeradorGrafo:
	...
	def pcv_genetico(self, tam_pop, geracoes, tam_torneio, prob_cruz, prob_mut):
			...
			for j in range(tam_torneio):
				...
				if random.random() <= prob_cruz:
					...
					while True:
						...
						ponto = random.randint(1, self.n - 1)
		...
		for individuo in range(1, tam_pop):
			if obter_custo(pop[individuo]) < obter_custo(melhor_individuo):
				melhor_individuo = pop[individuo][:]

		print('Melhor individuo: %s - Custo: %d' % (str(melhor_individuo), obter_custo(melhor_individuo)))


grafo = GeradorGrafo(30)
grafo.gerar_grafo()
print('RANDOM')
grafo.pcv_random(1000)
print('\nGENETICO')
grafo.pcv_genetico(tam_pop=1000, geracoes=1000, tam_torneio=1, prob_cruz=0.7, prob_mut=0.2)
